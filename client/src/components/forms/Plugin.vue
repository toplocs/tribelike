<template>
  <form @submit.prevent="addPlugin" class="space-y-6">
    ...
    <div>
      ...
      <div v-for="(slot, index) in formData.slots" :key="index" class="flex gap-2 mt-2">
        <TextInput v-model="slot.slot" placeholder="Slot name (e.g. InfoView)" class="flex-1" />
        <TextInput v-model="slot.component" placeholder="Component (e.g. Main)" class="flex-1" />
        <button @click.prevent="removeSlot(index)" class="text-red-500 text-sm">Remove</button>
      </div>
      <button @click.prevent="addSlot" class="text-blue-500 text-sm mt-1">+ Add Slot</button>
    </div>

    <!-- Paths -->
    <div>
      <label class="block text-sm font-semibold text-gray-900 dark:text-gray-100">Paths</label>
      <div v-for="(path, index) in formData.paths" :key="index" class="flex gap-2 mt-2">
        <TextInput v-model="path.path" placeholder="Path (e.g. location)" class="flex-1" />
        <TextInput v-model="path.component" placeholder="Component (e.g. CommunityView)" class="flex-1" />
        <button @click.prevent="removePath(index)" class="text-red-500 text-sm">Remove</button>
      </div>
      <button @click.prevent="addPath" class="text-blue-500 text-sm mt-1">+ Add Path</button>
    </div>

    <!-- Tabs -->
    <div>
      <label class="block text-sm font-semibold text-gray-900 dark:text-gray-100">Tabs</label>
      <div v-for="(tab, index) in formData.tabs" :key="index" class="flex gap-2 mt-2">
        <TextInput v-model="tab.value" placeholder="Tab name (e.g. Community)" class="flex-1" />
        <TextInput v-model="tab.href" placeholder="Href (e.g. location)" class="flex-1" />
        <button @click.prevent="removeTab(index)" class="text-red-500 text-sm">Remove</button>
      </div>
      <button @click.prevent="addTab" class="text-blue-500 text-sm mt-1">+ Add Tab</button>
    </div>

    <SubmitButton type="submit">Add Plugin</SubmitButton>
  </form>
</template>
