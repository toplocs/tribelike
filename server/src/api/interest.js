...
router.route('/').get((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { success, error } = yield (0, interest_1.findInterests)(req.query);
    if (success)
    ...
    var _a, _b;
    const { success, error } = yield (0, interest_1.createInterest)(req.body);
    ...
    const { success, error } = yield (0, interest_1.updateInterest)(req.body);
    if (success)
        return res.status(200).json(success);
    else
        return res.status(400).json(error);
...
router.route('/byId/:id').get((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { success, error } = yield (0, interest_1.getInterestById)(req.params);
    if (success)
        return res.status(200).json(success);
    else
        return res.status(400).json(error);
...
router.route('/add').put((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { success, error } = yield (0, interest_1.addInterest)(req.body);
    if (success)
        return res.status(200).json(success);
    else
        return res.status(400).json(error);
...
router.route('/remove').put((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { success, error } = yield (0, interest_1.removeInterest)(req.body);
    if (success)
        return res.status(200).json(success);
    else
        return res.status(400).json(error);
